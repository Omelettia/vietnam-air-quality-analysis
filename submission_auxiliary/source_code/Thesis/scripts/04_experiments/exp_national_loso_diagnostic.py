...
# Static/location features. Observable at a new site (fair in the generous
# diagnostic), but per-station constants that let random KFold identify the
# station. --features obs drops them (plus the region one-hots), mirroring the
# observation-only policy of the regional FEAT_OBS set.
STATIC_IDENTITY = {
    "latitude",
    "longitude",
    "elevation_m",
...
# Strong known-station configuration copied from exp_random_sample_kfold.py so
# LOSO can be rerun with the exact same model as the random-sample KFold
# headline; the fold scheme is then the only difference between the two runs.
XGB_KFOLD_MATCHED = dict(
    n_estimators=800,
    max_depth=8,
    learning_rate=0.05,
    subsample=0.8,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="full",
        help=(
            "'obs' drops the static/location features (lat/lon, DEM, region "
            "one-hots) for an observation-only control, mirroring the regional "
            "FEAT_OBS policy. Outputs get an _obsonly suffix."
        ),
    )
    parser.add_argument(
        "--config",
        ...
        default="default",
        help=(
            "'kfold' reruns LOSO with the exact strong model, raw-PM2.5 target and "
            "station weights of exp_random_sample_kfold.py (global pool only, "
            "outputs suffixed _kfold_config); overrides the three model flags."
        ),
    )
...
def prepare_data(obs_only: bool = False) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    feature_list = pd.read_csv(feature_path) if feature_path.exists() else None
    listed_features = (
        set(feature_list["feature"].astype(str)) if feature_list is not None else set()
    )
    required_columns = {"stationId", "ts", "PM2.5", "region"}
    usecols = (
        (lambda col: col in required_columns or col in listed_features)
    ...
    for col in candidate_cols:
        if col in EXCLUDE_FEATURES or col in NON_FEATURE_COLUMNS:
            continue
        if obs_only and col in STATIC_IDENTITY:
            continue
...
def train_predict(
    ...
    model.fit(x_all[train_idx], y_fit[train_idx], sample_weight=sample_weight)
...
def summarize(oof: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for spec in CONFIGS:
        ...
        pooled = r2_score(sub["y_true"], sub["y_pred"])
        summary_rows.append(
            {
                "config": config,
...
def write_report(
    ...
    kfold_config: bool = False,
) -> None:
    title = "INTERNAL LOSO NATIONAL DIAGNOSTICS"
    if kfold_config:
        title += " (KFOLD-MATCHED CONFIG CONTROL)"
    lines = [
        title,
    ...
    if kfold_config:
        lines += [
            "Purpose: controlled comparison against the random-sample KFold result.",
            "Model, raw-PM2.5 target and inverse-frequency station weights are",
            "identical to exp_random_sample_kfold.py; only the fold scheme differs",
            "(whole held-out stations instead of random rows). Global pool only.",
            "",
        ]
    lines += [
        "Feature policy",
    ...
    lines.extend(
        [
            ...
            "pipeline. See:",
            "",
            "Thesis/results/04_validation/report_red_river_delta.txt",
            "",
...
def main() -> None:
    ...
    kfold_cfg = args.config == "kfold"
    if kfold_cfg:
        params.update(XGB_KFOLD_MATCHED)

    print("=" * 80)
    print("NATIONAL LOSO DIAGNOSTIC" + (" (KFOLD-MATCHED CONFIG)" if kfold_cfg else ""))
    print("=" * 80)
    ...
    y_fit = y_true_all if kfold_cfg else y_log
    ...
    for fold_i, held_sid in enumerate(station_ids, start=1):
        ...
        for spec in (CONFIGS[:1] if kfold_cfg else CONFIGS):
            if spec["pool"] == "global":
            ...
            weight = None
            if kfold_cfg:
                tr_sids = pd.Series(sid_values[train_idx]).astype(str)
                cnt = tr_sids.value_counts()
                weight = (
                    len(train_idx) / len(cnt) / tr_sids.map(cnt)
                ).to_numpy(np.float32)
            pred = train_predict(
                x_all, y_fit, train_idx, test_idx, params,
                log_target=not kfold_cfg, sample_weight=weight,
            )
            rows = pd.DataFrame(
                {
                    "config": spec["config"],
    ...
    suffix = ("_kfold_config" if kfold_cfg else "") + (
        "_obsonly" if args.features == "obs" else ""
    )
    summary_csv = OUT_DIR / f"national_loso_diagnostic_summary{suffix}.csv"
    station_csv = OUT_DIR / f"national_loso_diagnostic_station_metrics{suffix}.csv"
    oof_csv = RAW_OUT_DIR / f"national_loso_diagnostic_oof{suffix}.csv"
    report_path = OUT_DIR / f"report_loso{suffix}.txt"
    ...
    write_report(summary, feature_cols, elapsed, report_path, kfold_cfg)
