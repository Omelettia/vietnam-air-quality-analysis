...
"""
import os, sys, io, zipfile, warnings
import numpy as np
import pandas as pd
...
QC_DIR = BASE / "Thesis" / "scripts" / "02_processing"
if str(QC_DIR) not in sys.path:
    sys.path.insert(0, str(QC_DIR))
from pm25_qc import pm25_quality_masks


def resolve_single_zip(pattern, label):
    matches = sorted((BASE / "data" / "gee_exports").glob(pattern))
    if not matches:
        raise FileNotFoundError(f"No {label} ZIP found with pattern {pattern}")
    if len(matches) > 1:
        print(f"  WARNING: multiple {label} ZIPs found; using newest by modified time")
        matches = sorted(matches, key=lambda p: p.stat().st_mtime)
    return matches[-1]

# ── station metadata ──
META = pd.read_csv(BASE / "Thesis" / "results" / "01_stations" / "station_selection_final.csv")
...
# ── GHAP ──
ghap_zip = resolve_single_zip("pm25-*.zip", "GHAP")
print(f"  GHAP ZIP: {ghap_zip.name}")
...
# ── GEOS-CF ──
hourly_zip = resolve_single_zip("hourly-*.zip", "GEOS-CF/MERRA-2")
print(f"  Hourly ZIP: {hourly_zip.name}")
geoscf_frames = []
merra2_frames = []
...
gt_path = BASE / "data" / "merged" / "unified_thesis.csv"
gt = pd.read_csv(gt_path, usecols=["ts", "PM2.5", "stationId"])
# The merged table holds all 121 stations; restrict ground truth to the 40 thesis stations.
_thesis40 = set(pd.read_csv(BASE / "Thesis" / "results" / "01_stations" / "station_selection_final.csv",
                            dtype={"stationId": str})["stationId"])
gt = gt[gt["stationId"].isin(_thesis40)].copy()
gt["ts"] = pd.to_datetime(gt["ts"])
gt = gt.dropna(subset=["PM2.5"])
qc_masks = pm25_quality_masks(gt, station_col="stationId", ts_col="ts",
                              value_col="PM2.5")
removed = qc_masks.any(axis=1)
if removed.any():
    print(f"  PM2.5 QC removed {int(removed.sum()):,} rows "
          f"({100 * removed.mean():.2f}%) before CTM comparison")
gt = gt[~removed].copy()
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 6: Additional diagnostics (bias correction, anomaly/ratio correlations)
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 80)
...
print("\n" + "=" * 80)
print("DONE — thesis conclusion: global CTM products are reference-only")
print("(GEOS-CF: strong seasonal bias; MERRA-2: weak hourly correlation).")
print("=" * 80)
