...
with rasterio.open(aod_file) as src:
    ...
    for _, row in stations.iterrows():
        # Station files are keyed by station NAME (colons stripped) — this is
        # the layout that build_unified.py indexes under data/station_aod/L2/.
        station_name = str(row["stationName"]).replace(":", "").strip()
        lon, lat = row["longitude"], row["latitude"]
        station_file = str(output_dir / f"{station_name}.csv")
