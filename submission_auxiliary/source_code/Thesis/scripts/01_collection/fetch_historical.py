"""
Stage 2 — download hourly observations from tedp.vn /api/data_hour/{record_id}
for every record in the per-station Stage-1 index (produced by fetch_id.py),
and write one CSV per station to data/stations/historical_full.

Single-run, resumable downloader:
 - N worker threads, each rate-limited to R req/s (total ~N*R req/s).
 - One writer thread appends parsed rows to per-station CSVs (header-aware,
   buffered append; no full reload).
 - Resume: existing output CSVs are read at startup and already-fetched
   Record_IDs are skipped, so an interrupted run can be restarted safely.
 - Ctrl-C drains in-flight requests and flushes pending buffers.
 - Full record schema parsed via detail_parser.py (pollutants, meteorology,
   VOCs, particle-size bins).

Usage:
  python Thesis/scripts/01_collection/fetch_historical.py \
      --inventory data/stations/historical_index \
      --output-dir data/stations/historical_full \
      --stations all \
...
HEADERS = {"User-Agent": "Mozilla/5.0 (thesis-data-pipeline)"}
...
def build_queue(inventory_dir, station_filter, output_dir):
    ...
    for path in sorted(glob.glob(os.path.join(inventory_dir, '*.csv'))):
        ...
        with open(path, 'r', encoding='utf-8', newline='') as f:
            reader = csv_mod.DictReader(f)
            for row in reader:
                rid = (row.get('Record_ID') or row.get('record_id') or '').strip()
                if not rid:
                    continue
...
def main():
    ...
    ap.add_argument('--inventory', default='data/stations/historical_index')
    ap.add_argument('--output-dir', default='data/stations/historical_full')
