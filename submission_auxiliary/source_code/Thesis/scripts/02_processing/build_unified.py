...
Main processing choices:
  - All stations are kept; QC is row-level via pm25_quality_masks
    (flatline, stuck-low, range checks).
  - station_type labels each station as LCS or KK.
  - A station needs at least 10% PM2.5 coverage to enter the table.

Merges: Envisoft hourly + OpenMeteo weather + Himawari AOD (L1+L2) + GPM rain + DEM
        + GEE/TROPOMI daily + MODIS temporal + static urban/source context.
Met policy: ERA5/Open-Meteo only — Temperature/Humidity/Pressure(_final) and
        WS_local/WD_local carry the gridded source, never in-situ sensors,
        so training, external validation and the map grid share one met source.
Feature engineering: AOD spatial gradients, temporal lags, physics correction,
                     rain features, wind components, temporal encodings and
                     regional-model daily anomaly/rolling fields.

Usage: python Thesis/scripts/02_processing/build_unified.py
"""
import io, sys, os, warnings, glob, unicodedata, time
import numpy as np
import pandas as pd
...
FEATURE_DIR = os.path.join(BASE, "Thesis", "scripts", "03_features")
if FEATURE_DIR not in sys.path:
    sys.path.insert(0, FEATURE_DIR)
from regional_feature_pipeline import enrich_unified_dataframe
...
gpm_idx = build_index("data/gpm/station_gis_extracted/*.csv", space_char="_")
...
for idx, (_, row) in enumerate(metadata.iterrows()):
    ...
    # Met policy: ERA5/Open-Meteo ONLY (deployable feature policy of the thesis).
    # In-situ met stays out of the model features entirely, so training,
    # external validation and the prediction grid all see the same met source;
    # the only station-derived model inputs are the fold-computed RFSI features.
    ...
    if "WS_om" in base.columns:
        base["WS_local"] = base["WS_om"]
    if "WD_om" in base.columns:
        base["WD_local"] = base["WD_om"]
...
# Attach all non-fold-dependent GEE/TROPOMI, MODIS temporal and static
# urban/source-context features to the same canonical table.  RFSI is the only
# model feature intentionally omitted here because it must be recomputed after
# excluding the held-out station in each LOSO fold.
print("Enriching canonical table with GEE/MODIS regional-model features...")
unified = enrich_unified_dataframe(unified, BASE, verbose=True)
