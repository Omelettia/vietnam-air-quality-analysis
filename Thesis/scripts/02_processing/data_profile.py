"""
Comprehensive data profile of unified_thesis_v4.csv for thesis defense.
...
# =====================================================================
P("# Data Profile: unified_thesis_v4.csv")
P()

df = pd.read_csv("data/merged/unified_thesis_v4.csv", dtype={"stationId": str})
...
EXCLUDE = {"31616865099255512061948816121",
...
P(f"- **Unique stations (after excluding 3 broken)**: {n_stn}")
...
FEAT_NN23_DOW = [
    # Meteorological
    "PBLH", "VC", "wind_u", "wind_v", "WS_local",
    ...
    # AOD
    "AOT_ffill_48h", "AOT_outer_mean", "AE", "RF",
    "hours_since_valid_AOT",
    # Building
    "building_area_1km",
    # Emission composite
    "smart_v1_center", "smart_v1_upwind", "smart_v1_max",
    "smart_v1_contrast", "smart_v1_upwind_x_VC_inv",
    # SO2/CO/HCHO standalone
    "so2_upwind", "so2_center", "so2_contrast",
    "co_upwind", "co_center", "hcho_center",
    "so2_upwind_x_VC_inv", "lst_anom_upwind_x_VC_inv",
    # Outer AOD
    "aod_outer_surface", "aod_outer_pm25", "aod_outer_x_VC_inv",
    "aod_outer_gradient", "aod_outer_day_mean",
    # Daily anomalies
    "so2_daily_anom", "co_daily_anom", "no2_daily_anom",
    "lst_day_anom", "lst_night_anom",
    "so2_anom_x_vc_inv", "co_anom_x_vc_inv", "lst_anom_x_vc_inv",
    # Weather persistence
    "PBLH_min_24h", "VC_min_24h", "stagnation_hours_12h",
    "temp_diurnal_anomaly",
    # RFSI
    "PM25_nn_idw", "PM25_nn1", "dist_nn1",
    "PM25_nn2", "PM25_nn3",
...
in_csv = [f for f in FEAT_NN23_DOW if f in df.columns]
not_in_csv = [f for f in FEAT_NN23_DOW if f not in df.columns]

P(f"Of the {len(FEAT_NN23_DOW)} model features:")
P(f"- **{len(in_csv)}** exist in the CSV (static/pre-computed)")
P(f"- **{len(not_in_csv)}** are computed at runtime by exp_definitive_v3.py")
P()
...
P(f"- **{n_stn} stations** after QC (37 kept, 3 excluded)")
P(f"- **{pm_nonnull:,} hourly PM2.5 observations** ({100*pm_nonnull/len(df):.1f}% non-null)")
P(f"- **{len(df.columns)} raw columns** in CSV; model uses 66 features (many computed at runtime)")
