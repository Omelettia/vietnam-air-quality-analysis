"""
Within-station predictability using the full enriched feature table.
Two runs:
  A) 40 thesis stations (KK)
  B) All eligible thesis and LCS-pass stations

For each station: 5-fold KFold with station's OWN data in training.
Features: full enrichment (met, AOD, emissions, temporal, building, terrain)
          — NO RFSI (that's spatial, not temporal).
Cleaning uses the shared thesis pm25_quality_masks pipeline so this diagnostic
is aligned with the final regional model.
"""
import argparse
...
parser = argparse.ArgumentParser(
    description="Rerun the within-station temporal diagnostic on the final data."
)
parser.add_argument(
    "--scope",
    choices=["thesis", "all"],
    default="thesis",
    help="Stations to evaluate. The thesis uses the 40 KK stations.",
)
parser.add_argument(
    "--max-iter",
    type=int,
...
QC_DIR = os.path.join(os.getcwd(), "Thesis", "scripts", "02_processing")
...
def hgb():
    return HistGradientBoostingRegressor(max_iter=ARGS.max_iter, max_depth=7,
...
# ── Enrichment (matching final pipeline) ─────────────────────────────────────
...
def clean_station(g):
    ...
    masks = pm25_quality_masks(g, station_col="stationId", ts_col="ts", value_col="PM2.5")
    out = g.loc[pm.notna() & ~masks.any(axis=1)].copy()
...
# Run B is optional. The thesis only reports the 40 KK station diagnostic.
if ARGS.scope == "all":
    ...
    res_all = res_thesis.copy()

# Save
out_path = "analysis/thesis_experiments/within_station_predictability.csv"
all_out_path = "analysis/thesis_experiments/within_station_predictability_all.csv"
thesis_out_path = "Thesis/results/03_model/within_station_predictability.csv"
report_path = "Thesis/results/03_model/report_within_station.txt"
os.makedirs(os.path.dirname(out_path), exist_ok=True)
os.makedirs(os.path.dirname(thesis_out_path), exist_ok=True)
res_thesis = res_thesis.copy()
res_thesis["group"] = "thesis"
res_thesis.to_csv(out_path, index=False)
res_thesis.to_csv(thesis_out_path, index=False)
print(f"\nSaved: {out_path}")
print(f"Saved: {thesis_out_path}")
if ARGS.scope == "all":
    res_all = res_all.copy()
    res_all["group"] = res_all["sid"].apply(
        lambda s: "thesis" if s in THESIS_SIDS else "lcs")
    res_all.to_csv(all_out_path, index=False)
    print(f"Saved: {all_out_path}")

def _summary_row(label, r):
    if not len(r):
        return f"  {label:<20} {0:>4} {'nan':>12} {'nan':>14} {'nan':>14}"
    return (
        f"  {label:<20} {len(r):>4} {r['wk_exog'].mean():>+12.3f} "
...
def _write_report():
    lines = [
        "=" * 80,
        "WITHIN-STATION PREDICTABILITY",
        "Train on each station's own data; no RFSI or neighbouring PM2.5.",
        "Shared PM2.5 QC: Thesis/scripts/02_processing/pm25_qc.py",
        f"Scope: {ARGS.scope}",
        f"Source CSV: {thesis_out_path}",
        "=" * 80,
        "",
        "Metric: 5-fold shuffled KFold R2 using exogenous features.",
        "This is a temporal-signal diagnostic, not spatial extrapolation.",
        "",
        f"40 KK thesis stations: n={len(res_thesis)}",
        f"  mean KFold R2    = {res_thesis['wk_exog'].mean():.3f}",
        f"  median KFold R2  = {res_thesis['wk_exog'].median():.3f}",
        f"  min KFold R2     = {res_thesis['wk_exog'].min():.3f}",
        f"  max KFold R2     = {res_thesis['wk_exog'].max():.3f}",
        f"  positive stations= {(res_thesis['wk_exog'] > 0).sum()}/{len(res_thesis)}",
        f"  mean temporal 70/30 R2 = {res_thesis['wt_exog'].mean():.3f}",
        "",
        "Per-tier median KFold R2 for 40 KK stations:",
    ]
    for t in ["t0", "t1", "t2", "t3"]:
        s = res_thesis[res_thesis["tier"] == t]
        if len(s):
            lines.append(
                f"  {t}: n={len(s):2d}, median={s['wk_exog'].median():.3f}, "
                f"mean={s['wk_exog'].mean():.3f}, pm_mean={s['pm'].mean():.1f}"
            )
    lines += [
        "",
        "Summary comparison:",
        "  Group                   n   kfold_mean   kfold_median  temporal_mean",
        "  " + "-" * 70,
        _summary_row("Thesis (KK)", res_thesis),
    ]
    if ARGS.scope == "all":
        lcs_only = res_all[~res_all["sid"].isin(THESIS_SIDS)]
        lines.append(_summary_row("LCS only", lcs_only))
        lines.append(_summary_row("All combined", res_all))
    lines += ["", f"Elapsed seconds: {time.time()-t0:.1f}", ""]
